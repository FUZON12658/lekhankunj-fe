...
export const Button = ({
    ...
    const buttonIconRight =
        iconRight === "placeholder" ? (
            ...
            <CartIconWhite className={`w-6 h-6 text-white`} />
        ) : null;

    // ✅ Add white variant support
    const whiteStyles =
        color === "white"
            ? style === "line"
    ...
    const buttonElement = (
        <button
            onClick={onClick}
            className={`btn text-btn uppercase px-3 h-10 flex justify-center items-center gap-2 rounded-sm ${buttonStyles} ${whiteStyles} ${buttonSize}`}
        >
            {buttonIconLeft}
            {text && <span>{text}</span>}
            {buttonIconRight}
